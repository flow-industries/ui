...
function statusIconAnimation(reduceMotion: boolean) {
...
function ValidatedInput({
  ...
  const iconAnimation = statusIconAnimation(!!useReducedMotion());
  return (
        ...
        <AnimatePresence initial={false}>
          {status === "checking" && (
            <motion.div
              ...
              {...iconAnimation}
            >
              <Loader2 className="size-4 animate-spin text-muted-foreground stroke-[3] motion-reduce:animate-none" />
            </motion.div>
          )}
          {status === "valid" && (
            <motion.div
