...
- **Dock** — Animated icon dock with hover popovers for label/extra content. Items support `default`, `secondary`, and `primary` variants, custom icons, and a slot for arbitrary popover content. Respects `prefers-reduced-motion`. Extracted from `flow-industries/talk` (`src/components/ui/dock.tsx`).
